case class Point(lat: Double, lon: Double):
  ...
  def distance(p1: Point, p2: Point): Double =
    ...
    if (p1.lat == p2.lat) && ((p1.lon < p2.lon && p1.lon <= lon && lon <= p2.lon) || (p2.lon < p1.lon && p2.lon <= lon && lon <= p1.lon))
    then lat * lat
    else if (p1.lon == p2.lon) && ((p1.lat < p2.lat && p1.lat <= lat && lat <= p2.lat) || (p2.lat < p1.lat && p2.lat < lat && lat < p1.lat))
    then lon * lon
    else
      ...
      if (
        math.min(p1.lat, p2.lat) <= iLat && iLat <= math.max(
          p1.lat,
          p2.lat
        ) && math
          .min(p1.lon, p2.lon) <= iLon && iLon <= math.max(p1.lon, p2.lon)
      ) then this.distance(Point(iLat, iLon))
      else default
...
case class Segment(bounds: Bounds, points: List[Point]):
  ...
  def distances(that: Segment) =
    if bounds.overlap(that.bounds) > 0 then List(0)
    else List(0)

  def extractSegments(that: Segment): (Set[Segment], Set[Segment]) =
    if this._1.overlap(that._1) == 0 then (Set(), Set(this))
    else
      case class State(
          ol_seg: Set[Segment],
          ol_points: List[Point],
          nol_seg: Set[Segment],
          nol_points: List[Point]
      )
      ...
      def next(s: State, p: Point): State =
        ...
        (isClose, s.ol_points, s.nol_points) match
          ...
          case (false, _, _) =>
            State(
              s.ol_seg + Segment.fromPoints(s.ol_points),
              List(),
              s.nol_seg,
              List(p)
            )
          case (true, _, List()) =>
            State(s.ol_seg, p :: s.ol_points, s.nol_seg, s.nol_points)
          case (true, _, _) =>
            State(
              s.ol_seg,
              List(p),
              s.nol_seg + Segment.fromPoints(s.nol_points),
              List()
            )
...
object Segment:
    ...
    def next(points: List[Point]): List[List[Point]] =
      points match {
        ...
        case p1 :: p2 :: rest => {
          ...
          otherSegs.foldLeft(List(List(p2, p1)))((a, seg) =>
            a match {
              case first :: Nil =>
                Segment.intersect(seg0, seg) match {
                  case None => List(seg._2 :: first)
                  case Some(iPoint) =>
                    List(
                      seg._2 :: iPoint :: Nil,
                      iPoint :: (first.init ++ List(iPoint)),
                      iPoint :: p1 :: Nil
                    )
                }
  ...
  def in(p: Point, segment: List[Point], threshold: Double) = {
    ...
    def closest(p1: Point, p2: Point) =
      if (distance(p1) < distance(p2)) p1 else p2
